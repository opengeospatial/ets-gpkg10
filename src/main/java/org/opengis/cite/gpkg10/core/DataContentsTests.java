 ...
 * GeoPackage. The {@code gpkg_contents} table describes the geospatial data contained in
 * the file.
 *
 ...
 * </p>
 * <ul>
 * <li><a href="http://www.geopackage.org/spec/#_contents" target= "_blank"> GeoPackage
 * Encoding Standard - Contents</a> (OGC 12-128r12)</li>
 * </ul>
 *
 * @author Luke Lambert
 */
public class DataContentsTests extends CommonFixture {
	...
	/**
	 * The columns of tables in a GeoPackage SHALL only be declared using one of the data
	 * types specified in table
	 * <a href= "http://www.geopackage.org/spec/#table_column_data_types">GeoPackage Data
	 * Types</a>.
	 *
	 * @see <a href="http://www.geopackage.org/spec/#_requirement-5" target= "_blank">File
	 * Contents - Requirement 5</a>
					...
					try (final Statement preparedStatement = this.databaseConnection.createStatement();
						...
						while (pragmaTableInfo.next()) {
							...
							final boolean correctDataType = ALLOWED_SQL_TYPES.contains(dataType)
									|| TEXT_TYPE.matcher(dataType).matches() || BLOB_TYPE.matcher(dataType).matches();

							assertTrue(correctDataType,
									ErrorMessage.format(ErrorMessageKeys.INVALID_DATA_TYPE, dataType, tableName));
						}
					}
				}
			}
	 ...
	 * <a href="http://www.geopackage.org/spec/#gpkg_contents_cols">Contents Table or View
	 * Definition</a> and
	 * <a href= "http://www.geopackage.org/spec/#gpkg_contents_sql">gpkg_contents Table
	 * Definition SQL</a>.
	 *
	...
	public void contentsTableDefinition() throws SQLException {
		try {
			...
			contentColumns.put("last_change",
			...
			TableVerifier.verifyTable(this.databaseConnection, "gpkg_contents", contentColumns,
	...
	/**
	 * The {@code table_name} column value in a {@code gpkg_contents} table row SHALL
	 * contain the name of a SQLite table or view.
	 *
	...
	public void contentsTablesExist() throws SQLException {
		final String query = "SELECT DISTINCT table_name " + "FROM  gpkg_contents "
		...
		try (final Statement statement = this.databaseConnection.createStatement();
			...
			assertTrue(invalidContentsTableNames.isEmpty(), ErrorMessage
				.format(ErrorMessageKeys.CONTENT_TABLE_DOES_NOT_EXIST, String.join(", ", invalidContentsTableNames)));
		}
	}

	/**
	 * Values of the {@code gpkg_contents} table {@code last_change} column SHALL be in
	 * <a href="http://www.iso.org/iso/catalogue_detail?csnumber=40874">ISO 8601 </a>
	 * format containing a complete date plus UTC hours, minutes, seconds and a decimal
	 * fraction of a second, with a 'Z' ('zulu') suffix indicating UTC.
	 *
	...
	public void timestampFormat() throws SQLException {
		try (final Statement statement = this.databaseConnection.createStatement();
				final ResultSet resultSet = statement
					.executeQuery("SELECT last_change, table_name FROM gpkg_contents;")) {
			while (resultSet.next()) {
	...
	/**
	 * Values of the {@code gpkg_contents} table {@code srs_id} column SHALL reference
	 * values in the {@code gpkg_spatial_ref_sys} table {@code
	 * srs_id} column.
	 *
	...
	public void srsIdReferencesSrsTable() throws SQLException {
		try (final Statement statement = this.databaseConnection.createStatement();
			...
			assertTrue(!resultSet.next(), ErrorMessage.format(ErrorMessageKeys.BAD_CONTENTS_TABLE_SRS_FOREIGN_KEY));
		}
	}
	...
	private static final List<String> ALLOWED_SQL_TYPES = Arrays.asList("BOOLEAN", "TINYINT", "SMALLINT", "MEDIUMINT",
			"INT", "FLOAT", "DOUBLE", "REAL", "TEXT", "BLOB", "DATE", "DATETIME", "GEOMETRY", "POINT", "LINESTRING",
			"POLYGON", "MULTIPOINT", "MULTILINESTRING", "MULTIPOLYGON", "GEOMETRYCOLLECTION", "INTEGER");
